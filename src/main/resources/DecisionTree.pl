...
/*
discoverPreys(+X, -Y, -Path)
Giving as input x simulates, through a set of rules, the behavior of a binary tree, finding all the preys Y and calculating for each the best route to get there.
*/
discoverPreys(X, Y, Path) :-
...
/*
discoverPartners(+X, -Y, -Path)
Giving as input x simulates, through a set of rules, the behavior of a binary tree, finding all the partners Y and calculating for each the best route to get there.
...
path(PosX, PosY, Path) :-
	 ...
	 path(NewPosX, PosY, CurrentPath), add(Direction, CurrentPath, Path).
path(PosX, PosY, Path) :-
	PosX == PosY, 
	Path = [].
