package it.unibo.pps.ese.entitywatcher
...
class Stalker(consolidatedState: ReadOnlyEntityRepository) {
  ...
  def report: ResultEra = {
    ...
    var me: Point = null
    var target: EntityTimedRecord = null
    ...
    if (stalked.isDefined) {
      ...
      target = consolidatedState.entitiesInEra(currentEra).find(x => x.id == stalked.get) get
