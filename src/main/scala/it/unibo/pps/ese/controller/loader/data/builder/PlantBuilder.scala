...
object PlantBuilder {
  ...
  def apply(): PlantBuilder[EmptyPlant] =
    new PlantBuilderImpl[EmptyPlant](None, None, None, None, None, None, None, None)
  ...
  private class PlantBuilderImpl[T <: PlantStatus] (height: Option[Double],
    ...
    def setHeight(height: Double): PlantBuilder[T with PlantWithHeight] =
      new PlantBuilderImpl(Some(height), nutritionalValue, hardness, availability, name, geneLength,
        alleleLength, reign)

    def setNutritionalValue(nutritionalValue: Double): PlantBuilder[T with PlantWithNutritionalValue] =
      new PlantBuilderImpl(height, Some(nutritionalValue), hardness, availability, name, geneLength,
        alleleLength, reign)

    def setHardness(hardness: Double): PlantBuilder[T with PlantWithHardness] =
      new PlantBuilderImpl(height, nutritionalValue, Some(hardness), availability, name, geneLength,
        alleleLength, reign)

    def setAvailability(availability: Double): PlantBuilder[T with PlantWithAvailability] =
      new PlantBuilderImpl(height, nutritionalValue, hardness, Some(availability), name, geneLength,
        alleleLength, reign)

    def setName(name: String): PlantBuilder[T with PlantWithName] =
      new PlantBuilderImpl(height, nutritionalValue, hardness, availability, Some(name), geneLength,
        alleleLength, reign)

    def setGeneLength(geneLength: Int): PlantBuilder[T with PlantWithGeneLength] =
      new PlantBuilderImpl(height, nutritionalValue, hardness, availability, name, Some(geneLength),
        alleleLength, reign)

    def setAlleleLength(alleleLength: Int): PlantBuilder[T with PlantWithAlleleLength] =
      new PlantBuilderImpl(height, nutritionalValue, hardness, availability, name, geneLength,
    ...
    override def tryBuildComplete: Try[CompletePlantData] = {
      status.tpe match {
        case t if t <:< typeOf[FullPlant] =>
          Success(new PlantDataImpl(height, nutritionalValue, hardness, availability, name.get, geneLength,
    ...
    def buildComplete(implicit ev: T =:= FullPlant): CompletePlantData = {
      new PlantDataImpl(height, nutritionalValue, hardness, availability, name.get, geneLength,
        alleleLength, reign) with CompletePlantData
    }
    ...
    def build(): PartialPlantData = {
      ...
      tryBuildComplete match {
        case Success(value) =>
          value
        case Failure(_) =>
          PlantDataImpl(height, nutritionalValue, hardness, availability, name.get, geneLength,
            alleleLength, reign)
      }
    }
  }
