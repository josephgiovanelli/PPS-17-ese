...
object CustomGeneData {
  def apply(gene: Gene, alleles: Iterable[AlleleData]): CustomGeneData = new CustomGeneDataImpl(gene, alleles)

  private class CustomGeneDataImpl(gene: Gene, alleles: Iterable[AlleleData])
    extends {
